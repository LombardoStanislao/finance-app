...
export default function InvestmentsPage({ onBack, onOpenSettings, onOpenGuide, primaryColor }: InvestmentsPageProps) {
    ...
    const sellNetDetails = (() => {
        ...
        const pureGrossProceeds = round2(moneyVal)
        const realizedPL = round2(pureGrossProceeds - costBasis)
            ...
            if (editingId) {
                ...
                if (fees > 0) {
                    ...
                    await supabase.from('transactions').insert({
                        ...
                        type: 'expense',
                        category_id: commCatId,
                        description: `Commissioni Vendita: ${selectedInvestment.name}`,
                        date: new Date().toISOString(),
                        investment_id: selectedInvestment.id,
                        is_work_related: false,
                        is_recurring: false
    ...
    async function handleDeleteTransaction(tx: Transaction) {
        ...
        try {
            const txTime = new Date(tx.created_at || tx.date).getTime()
            const timeStart = new Date(txTime - 2000).toISOString()
            const timeEnd = new Date(txTime + 2000).toISOString()

            // 1. Fetch Atomic Group
            const { data: siblings } = await supabase
                .from('transactions')
                .select('*')
                .eq('investment_id', selectedInvestment.id)
            ...
            for (const sib of siblings) {
                if (sib.type === 'transfer') {
                    totalInvestedToRevert += sib.amount
                }
                if ((sib as any).asset_quantity) {
                    totalQtyToRevert += (sib as any).asset_quantity
                }
            }

            const newQty = roundQty(currentQty - totalQtyToRevert)
            const newInvested = round2(currentInvested + totalInvestedToRevert)
    ...
    const activeInvestments = investments.filter(i => (i.quantity || 0) > 0)
    const closedInvestments = investments.filter(i => (i.quantity || 0) <= 0)

    const totalAssets = activeInvestments.reduce((sum, item) => sum + (item.current_value || 0), 0)
    const totalInvested = activeInvestments.reduce((sum, item) => sum + (item.invested_amount || 0), 0)
        ...
        <div className="min-h-screen bg-gray-50 pb-24">
            ...
            <div className="max-w-md mx-auto px-4 py-6 space-y-8">
                ...
                <div className="space-y-8">
                    {loading ? <div className="space-y-4 animate-pulse"><div className="h-32 bg-gray-200 rounded-2xl" /></div> : activeInvestments.length === 0 ? <div className="text-center py-10 text-gray-400">Nessun investimento attivo</div> : (
                        categoriesConfig.map((cat) => {
                            const categoryItems = activeInvestments.filter(item => item.type === cat.type)
                ...
                {/* ARCHIVIO POSIZIONI CHIUSE */}
                {closedInvestments.length > 0 && (
                    <div className="mt-12 space-y-4">
                        <div className="flex items-center gap-2 mb-4 px-2">
                            <History className="w-5 h-5 text-gray-400" />
                            <h2 className="text-sm font-bold text-gray-500 uppercase tracking-widest">Archivio Posizioni Chiuse</h2>
                        </div>
                        <div className="space-y-3 opacity-60 hover:opacity-100 transition-opacity">
                            {closedInvestments.map(item => {
                                ...
                                <div key={item.id} onClick={() => openDetail(item)} className="bg-gray-100 p-4 rounded-xl border border-gray-200 flex items-center justify-between group active:scale-[0.99] transition-transform cursor-pointer">
                                    <div className="min-w-0 pr-4">
                                        <div className="flex items-center gap-2">
                                            <p className="font-bold text-gray-600 text-base line-through decoration-gray-300">{item.name || item.type}</p>
                                            <span className="bg-gray-200 text-gray-500 text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wide">Chiusa</span>
                                        </div>
