...
export default function Transactions({ onBack, onOpenSettings, primaryColor }: TransactionsProps) {
  ...
  async function loadTransactions() {
    try {
      ...
      if (filterCategory !== 'all') {
        const childIds = flatCategories.filter(c => c.parent_id === filterCategory).map(c => c.id)
        const allRelevantIds = [filterCategory, ...childIds]
  ...
  function getCategoryName(categoryId: string | null): string {
    ...
    return flatCategories.find(c => c.id === categoryId)?.name || 'Sconosciuta'
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-md mx-auto px-4 py-4 space-y-4">
          ...
          {filterType !== 'transfer' && (
            <div className="relative animate-in fade-in slide-in-from-top-2">
              ...
              <select
                ...
                className="w-full bg-gray-50 border-none rounded-xl py-2.5 pl-10 pr-4 text-sm font-medium text-gray-700 focus:ring-2 focus:ring-blue-500 outline-none transition-all appearance-none"
              >
                <option value="all">Tutte le Categorie</option>
                
                {(filterType === 'all' || filterType === 'income') && categoryTree.some(c => c.type === 'income') && (
                  <optgroup label="⤵ ENTRATE">
                    {categoryTree
                      .filter(cat => cat.type === 'income')
                      .map(cat => (
                        <React.Fragment key={cat.id}>
                          <option value={cat.id} className="font-bold text-emerald-800 bg-emerald-50/50">• {cat.name}</option>
                          {cat.children?.map(child => (
                            <option key={child.id} value={child.id} className="text-emerald-700 bg-emerald-50/50">&nbsp;&nbsp;&nbsp;&nbsp;↳ {child.name}</option>
                          ))}
                        </React.Fragment>
                      ))
                    }
                ...
                {(filterType === 'all' || filterType === 'expense') && categoryTree.some(c => c.type === 'expense') && (
                  <optgroup label="⤴ USCITE">
                    {categoryTree
                      .filter(cat => cat.type === 'expense')
                      .map(cat => (
                        <React.Fragment key={cat.id}>
                          <option value={cat.id} className="font-bold text-rose-800 bg-rose-50/50">• {cat.name}</option>
                          {cat.children?.map(child => (
                            <option key={child.id} value={child.id} className="text-rose-700 bg-rose-50/50">&nbsp;&nbsp;&nbsp;&nbsp;↳ {child.name}</option>
                          ))}
                        </React.Fragment>
                      ))
                    }
        ...
        {/* SOMMARIO RICERCA */}
        {!loading && transactions.length > 0 && (filterType !== 'all' || filterCategory !== 'all' || filterSearch.trim() !== '' || dateFrom !== '' || dateTo !== '') && (
          <div className="bg-gray-900 rounded-2xl p-4 shadow-lg text-white grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-2">
             <div className="space-y-1">
               <span className="text-[10px] uppercase font-bold text-gray-400 block">Totale Entrate</span>
               <span className="text-xl font-black text-emerald-400">+{formatCurrency(transactions.filter(t => t.type === 'income').reduce((acc, t) => acc + Math.abs(t.amount), 0))}</span>
             </div>
             <div className="space-y-1 text-right">
               <span className="text-[10px] uppercase font-bold text-gray-400 block">Totale Uscite</span>
               <span className="text-xl font-black text-rose-400">-{formatCurrency(transactions.filter(t => t.type === 'expense').reduce((acc, t) => acc + Math.abs(t.amount), 0))}</span>
             </div>
          </div>
        )}
